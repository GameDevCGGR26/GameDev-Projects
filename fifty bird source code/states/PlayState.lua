...
function PlayState:update(dt)
    if scrolling == true then
        ...
        if self.timer > math.random(2, 50) then
            ...
            -- and no lower than a gap length (50 pixels) from the bottom
            local y = math.max(-PIPE_HEIGHT + 20, math.min(self.lastY + math.random(-20, 20), (VIRTUAL_HEIGHT - 80 - PIPE_HEIGHT)))
            self.lastY = y
